...
def anima(j):
   ...
   # RArrow.set_data(RArrowRX + X[j], RArrowRY + Y[j])
    return P, Vline, VArrow, Vline2, WArrow, Vline4,
#Vline3,RArrow,
...
Vline4, = ax1.plot([X[0], X[0] + (Y[0] + VY[0]) * Ro[0]/((Y[0] + VY[0])**2 +
         (X[0] + VX[0])**2)**0.5], [Y[0], Y[0] - (X[0] + VX[0]) * Ro[0]/
                                    ((Y[0] + VY[0])**2 + (X[0] + VX[0])**2)**0.5], 'b')  # vector of radius of curvature

ArrowX = np.array([-0.1 * R_, 0, -0.1 * R_])  # arrow of speed
ArrowY = np.array([0.05 * R_, 0, -0.05 * R_])
ArrowWX = np.array([-0.05 * R_, 0, 0.05 * -R_])  # arrow of acceleration
ArrowWY = np.array([0.05 * R_, 0, 0.05 * -R_])
ArrowRX = np.array([-0.05 * R_, 0, -0.05 * R_])  # arrow of radius vector
...
anim = FuncAnimation(fig, anima, frames=2000, interval=80, blit=True)
